...
int main(void)
{
  ...
  while (1)
  {
	...
	// show_a_random_number();
	  ...
	  HAL_TIM_Base_Stop_IT(&htim16);  // LED-D1 toggle according to Timer16
	  HAL_TIM_Base_Stop_IT(&htim6);  // LED-D2 toggle according to Timer6
	  HAL_TIM_Base_Stop_IT(&htim7);  // LED-D3 toggle according to Timer7
	  got_start();
      ...
      HAL_TIM_Base_Start_IT(&htim16);  // LED-D1 toggle according to Timer16
      HAL_TIM_Base_Start_IT(&htim6);  // LED-D2 toggle according to Timer6
      HAL_TIM_Base_Start_IT(&htim7);  // LED-D3 toggle according to Timer7
	// Display_Waiting();
